...
def extract_feats_from_pickle(input_path, output_path,label_column='ProdName'):
    in_DF = pd.read_pickle(input_path)
    feats_DF = feex.extractVarsForML(in_DF.loc[:5,:])
    feats_DF = pd.concat([feats_DF,in_DF.loc[:5,label_column]], axis=1)
    feats_DF.to_pickle(output_path)
    return feats_DF

def split_dataset(feats_DF,label_column='ProdName'):
...
def prepareTrainingData(output_path, input_path='', label_column='ProdName', extract=False):
    if extract:
